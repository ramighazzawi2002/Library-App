...
function BookCatalog() {
  ...
  const getData = async () => {
    try{
      const response = await axios.get("https://booklibrary-a5b1e-default-rtdb.firebaseio.com/books.json");
      setAllBooks(response.data);
      const notDeleted = response.data.filter(el => el.isDeleted === false)
      setBooks(notDeleted);
    }
    catch(error){
      console.log(error);
  ...
  const addBook = async () => {
    try{
      const response = await axios.put(`https://booklibrary-a5b1e-default-rtdb.firebaseio.com/books/${allBooks.length}.json`, {id:allBooks.length+1, author : addBookData.authorName, genre : addBookData.genre, language : addBookData.language, pages : addBookData.pages, publishedYear : addBookData.publishedYear, title : addBookData.bookName, isDeleted : false});
    }
    catch(error){
      console.log(error);
    }
  ...
  const editBook = async id => {
    try{
      const response = await axios.patch(`https://booklibrary-a5b1e-default-rtdb.firebaseio.com/books/${id}.json`, {author : editBookData.authorName, genre : editBookData.genre, language : editBookData.language, pages : editBookData.pages, publishedYear : editBookData.publishedYear, title : editBookData.bookName, isDeleted : false});
    }
    catch(error){
      console.log(error);
    }
  ...
  const deleteBook = async id => {
    try{
      const response = await axios.patch(`https://booklibrary-a5b1e-default-rtdb.firebaseio.com/books/${id}.json`, {isDeleted : true});
    }
    catch(error){
      console.log(error);
    }
  ...
  const restoreALlBooks = async () => {
    try{
      ...
      const response = await axios.patch(`https://booklibrary-a5b1e-default-rtdb.firebaseio.com/books/${i}.json`, {isDeleted : false});
    }
  }
    catch(error){
      console.log(error);
            ...
            <form className="p-4 md:p-5">
                <div className="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                        <label for="category" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Genre</label>
                        <input type="text" name="price" id="category" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Genre" required="" onChange={(e) => setEditBookData({...editBookData, genre: e.target.value})} />

                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="year" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Published Year</label>
                        <input type="text" name="price" id="year" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Published Year" required="" onChange={(e) => setEditBookData({...editBookData, publishedYear: e.target.value})} />
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="language" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Language</label>
                        <input type="text" name="price" id="language" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Language" required="" onChange={(e) => setEditBookData({...editBookData, language: e.target.value})} />
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="pages" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pages</label>
                        <input type="text" name="price" id="pages" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Pages" required="" onChange={(e) => setEditBookData({...editBookData, pages: e.target.value})}/>
                    </div>
                </div>
...
<div id="crud-modal" tabindex="-1" aria-hidden="true" className={`${openAddMOdal ? "block" : "hidden"} overflow-y-auto  overflow-x-hidden fixed top-0 left-0 right-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full`}>
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:left-[120%] sm:top-20">
            ...
            <form className="p-4 md:p-5">
                <div className="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                        <label for="category" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Genre</label>
                        <input type="text" name="price" id="category" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Genre" required="" onChange={(e) => setAddBookData({...addBookData, genre: e.target.value})} />

                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="year" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Published Year</label>
                        <input type="text" name="price" id="year" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Published Year" required="" onChange={(e) => setAddBookData({...addBookData, publishedYear: e.target.value})} />
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="language" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Language</label>
                        <input type="text" name="price" id="language" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Language" required="" onChange={(e) => setAddBookData({...addBookData, language: e.target.value})} />
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <label for="pages" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pages</label>
                        <input type="text" name="price" id="pages" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Pages" required="" onChange={(e) => setAddBookData({...addBookData, pages: e.target.value})}/>
                    </div>
                </div>
    ...
    {books.map(el => {
      ...
      <div className="mt-4 text-lg ">
        ...
        <p>
          <span className="font-semibold">Published Year:</span> {el.publishedYear}
        </p>
        <p>
          <span className="font-semibold">Genre:</span> {el.genre}
        </p>
        <p>
          <span className="font-semibold">Language:</span> {el.language}
        </p>
        <p>
          <span className="font-semibold">Pages:</span> {el.pages}
        </p>
      </div>
    ...
    <button className=" fixed right-3 bottom-36 z-50 bg-green-500 p-1 w-16 h-16 rounded-[50%] block text-white hover:bg-green-600 transition-all" onClick={() => restoreALlBooks()}>Restore</button>
    <button className=" fixed right-3 bottom-56 z-50 bg-green-500 p-1 w-16 h-16 rounded-[50%] block text-white hover:bg-green-600 transition-all" onClick={() => setOpenAddModal(true)}>add</button>
    </div>
  );
}
